...
async def health():
    return {"ok": True, "version": "5.12.1", "providers": ["tmdb", "vidsrc", "4khdhub", "hubcloud", "ytmusic", "jiosaavn", "deezer", "piped", "lrclib", "moviebox-api", "netmirror", "vixsrc"]}
...
async def tools_web_search(q: str = Query(..., min_length=1), max_results: int = Query(8, ge=1, le=20)):
    """DuckDuckGo instant + related topics (no key)."""
...
@app.get("/lyrics/lrcget", tags=["Lyrics"])
async def lyrics_lrcget(track_name: str = Query(...), artist_name: str = ""):
    """LRCLIB search (synced + plain)."""
    return await lyrics_musixmatch(title=track_name, artist=artist_name)

...
async def health():
    return {"ok": True, "version": "5.41.0", "creator": "shawon", "providers": ["hindianime", "ytmusic", "deezer", "jiosaavn", "tmdb"]}
...
# FastMusic / ClulesVibe — MUST ship fastmusic.py next to api.py
try:
    from fastmusic import router as fastmusic_router
    app.include_router(fastmusic_router)
except Exception as _fm_err:
    import sys, traceback
    print("WARNING: fastmusic.py missing or broken — /download /search will 404", _fm_err, file=sys.stderr)
    traceback.print_exc()

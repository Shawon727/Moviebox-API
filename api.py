...
async def health():
    return {"ok": True, "version": "5.37.0", "creator": "shawon", "providers": ["hindianime", "ytmusic", "deezer", "jiosaavn", "tmdb"]}
...
async def fm_thumbnail_hd(
    ...
    """Drop-in for: https://youtube-data.deno.dev/thumbnailHD?id=VIDEO_ID

    Returns HD album art for Glide/ExoPlayer notification.
    Tries maxresdefault → hqdefault → mqdefault.
    """
    from fastapi.responses import RedirectResponse
    vid = (id or "").replace("yt:", "").strip()
    if not re.match(r"^[\w-]{6,20}$", vid):
        raise HTTPException(400, "invalid video id")
    # prefer maxres; clients that 404 can fall back — we probe first
    candidates = [
    ...
    # Instant redirect — no HEAD probe (fast for Glide)
    chosen = candidates[0]
    if redirect:
    ...
    return {
        ...
        "thumbnail": chosen,
        "thumbnailHD": chosen,
        "thumbnail2": candidates[2] if len(candidates) > 2 else candidates[-1],
        "provider": "ytimg",
    }

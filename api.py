...
async def health():
    return {"ok": True, "version": "5.15.3", "providers": ["tmdb", "vidsrc", "4khdhub", "hubcloud", "ytmusic", "jiosaavn", "deezer", "piped", "lrclib", "moviebox-api", "netmirror", "vixsrc"]}
...
async def anime_stream(
    ...
    """Resolve HindiAnime HLS — returns absolute master + qualities + audio + CORS play_url."""
    ...
    stream = None
    master_body = ""
    hosts = [HA_BASE, HA_STREAM] if "HA_STREAM" in globals() or True else [HA_BASE]
    # Prefer www (currently serves m3u8); stream.* as fallback
    host_list = ["https://www.hindianime.site", "https://stream.hindianime.site"]

    if hash:
        async with httpx.AsyncClient(timeout=20.0, follow_redirects=True) as client:
            # Always prefer stream.* — www serves master but sub-playlists/segments become SPA HTML / 410
            for host in host_list:
                ...
                try:
                    ...
                    if r.status_code == 200 and body.lstrip().startswith("#EXTM3U"):
                        stream = candidate
                        master_body = body
                        if "stream.hindianime.site" in host:
                            break  # best host
                        # keep www only as temporary fallback; continue looking for stream.*
                except Exception:
                    continue
            # Normalize final stream URL to stream host
            if stream and "www.hindianime.site" in stream:
                stream = stream.replace("www.hindianime.site", "stream.hindianime.site")

    if not stream and hash:
        # still expose constructed URL so client can retry
        stream = f"https://stream.hindianime.site/api/proxy/master.m3u8?hash={hash}"

    if not stream:
        raise HTTPException(502, "Could not resolve stream")

    # Parse qualities + audio from master playlist
    qualities: List[dict] = []
    ...
    for i, line in enumerate(lines):
        if line.startswith("#EXT-X-MEDIA:") and "TYPE=AUDIO" in line:
            ...
            if uri_m:
                u = uri_m.group(1)
                if u.startswith("/"):
                    u = "https://stream.hindianime.site" + u
                elif "www.hindianime.site" in u:
    ...
    # CORS-safe play URL via our HLS proxy (rewrites segments)
    try:
    ...
    return {
        ...
        "type": "hls",
        "provider": "hindianime",
        "note": "Use play_url in web player (CORS). stream_url/direct_url is the absolute master.m3u8 for VLC.",
    }

...
async def health():
    return {"ok": True, "version": "5.22.0", "providers": ["tmdb", "vidsrc", "4khdhub", "hubcloud", "ytmusic", "jiosaavn", "deezer", "piped", "lrclib", "moviebox-api", "netmirror", "vixsrc"]}
...
async def music_yt_play(video_id: str):
    """Stream info for a YouTube / YT Music track.

    Priority: yt-dlp (googlevideo CDN) → Piped → Invidious → embed fallback.
    """
    ...
    # 1) yt-dlp (same path that powers /dl/ytmp3 on Vercel)
    try:
        data = await asyncio.to_thread(_ytdlp_info, watch)
        if data.get("ok"):
            title = data.get("title") or title
            thumb = data.get("thumbnail") or thumb
            duration = data.get("duration")
            provider = data.get("extractor") or "yt-dlp"
            for f in data.get("formats") or []:
                if not f.get("url"):
                    continue
                kind = (f.get("kind") or "").lower()
                entry = {
                    "label": f.get("label") or f.get("id") or "stream",
                    "url": f["url"],
                    "type": f.get("ext") or "mp4",
                    "format": "AUDIO" if "audio" in kind and "video" not in kind else ("VIDEO" if "video" in kind else "STREAM"),
            ...
            if data.get("url") and not audio_streams and not video_streams:
                audio_streams.append({"label": "best", "url": data["url"], "type": data.get("ext") or "m4a", "format": "AUDIO"})
    except Exception as e:
        errors.append(f"ytdlp:{type(e).__name__}")

    # 2) Piped multi-instance
    if not audio_streams and not video_streams:
        try:
            piped = await _piped_yt_streams(video_id)
            if piped.get("ok") and piped.get("formats"):
                provider = "piped"
                title = piped.get("title") or title
                for f in piped["formats"]:
                    if not f.get("url"):
                        continue
                    kind = (f.get("kind") or "").lower()
                    entry = {
                        "label": f.get("label") or "stream",
                        "url": f["url"],
                        "type": f.get("ext") or "webm",
                        "format": "AUDIO" if "audio" in kind and "video" not in kind else "VIDEO",
                    }
                    (audio_streams if entry["format"] == "AUDIO" else video_streams).append(entry)
        except Exception as e:
            errors.append(f"piped:{type(e).__name__}")

    # 3) Invidious (may be down)
    if not audio_streams and not video_streams:
        try:
            data = await _invidious_get(f"/api/v1/videos/{video_id}")
            provider = "invidious"
            title = data.get("title") or title
            thumb = _yt_thumb(video_id, data.get("videoThumbnails")) if "_yt_thumb" in dir() else thumb
            duration = data.get("lengthSeconds") or duration
            for f in (data.get("adaptiveFormats") or []):
                if not isinstance(f, dict) or not f.get("url"):
                    continue
                tpe = (f.get("type") or "").lower()
                if tpe.startswith("audio/") or "audio" in tpe:
                    audio_streams.append({
                        "label": str(f.get("bitrate") or f.get("quality") or "audio"),
                        "bitrate": f.get("bitrate"),
                        "url": f.get("url"),
                        "type": f.get("type"),
                        "format": "AUDIO",
                    })
                elif tpe.startswith("video/"):
            ...
            for f in (data.get("formatStreams") or []):
                if isinstance(f, dict) and f.get("url"):
                    video_streams.append({
                        "label": f.get("qualityLabel") or "mp4",
                        "url": f.get("url"),
                        "type": f.get("type"),
                        "format": "MP4",
                    })
        except Exception as e:
            errors.append(f"invidious:{type(e).__name__}:{e}")

    def _br(x):
        try:
            return int(re.sub(r"\D", "", str(x.get("bitrate") or x.get("label") or "0")) or 0)
        except Exception:
            return 0

    audio_streams.sort(key=_br, reverse=True)
    best_audio = audio_streams[0]["url"] if audio_streams else None
    best_video = video_streams[0]["url"] if video_streams else None

    if best_audio:
        sources.append({"type": "audio", "provider": provider or "yt", "label": "Best audio", "url": best_audio, "play_url": best_audio, "format": "AUDIO"})
    if best_video:
        sources.append({"type": "video", "provider": provider or "yt", "label": "Best video", "url": best_video, "play_url": best_video, "format": "VIDEO"})
    # always offer embed as last resort so UI never fully breaks
    embed = f"https://www.youtube.com/embed/{video_id}?autoplay=1&rel=0"
    sources.append({"type": "embed", "provider": "youtube", "label": "YouTube embed", "url": embed, "play_url": embed, "format": "EMBED"})

    ok = bool(best_audio or best_video)
    return {
        "ok": ok,
        "video_id": video_id,
        "title": title,
        ...
        "watch_url": watch,
        "download_url": watch,
        "provider": provider or ("embed" if not ok else "yt"),
        "errors": errors or None,
        "note": "Use audio_url / sources[0].url for players. Tokens in googlevideo URLs expire — re-fetch when needed.",
    }

...
async def health():
    return {"ok": True, "version": "5.35.0", "creator": "shawon", "providers": ["hindianime", "ytmusic", "deezer", "jiosaavn", "tmdb"]}
...
# =============================================================================
# FastMusic / PaxSenix-compatible YouTube Music API (unlimited, native InnerTube)
# Matches Sketchware PaxSenix music app + RapidAPI youtube-music-api3 shapes
# App used: youtube-data.deno.dev (dead) + RapidAPI — replaced with native CDN
# Fields expected by app player: videoId, title, author, thumbnail, url
# =============================================================================

def _fm_song(video_id: str, title: str = "", author: str = "", thumb: str = None, **extra) -> dict:
    """Song card shape used by PaxSenix Sketchware player (ExoPlayer service)."""
    vid = (video_id or "").replace("yt:", "").strip()
    thumb = thumb or (f"https://i.ytimg.com/vi/{vid}/hqdefault.jpg" if vid else None)
    return {
        "videoId": vid,
        "video_id": vid,
        "id": vid,
        ...
        "watch_url": f"https://music.youtube.com/watch?v={vid}" if vid else None,
        "play_url": f"/fm/play?id={vid}" if vid else None,
        "download_url": f"/fm/download?id={vid}" if vid else None,
        "provider": "fastmusic",
...
async def fm_search(
    q: str = Query(..., min_length=1, description="query"),
    type: str = Query("song", description="song|videos|albums|artists|playlists"),
    limit: int = Query(20, ge=1, le=50),
):
    """Search Music — RapidAPI/PaxSenix compatible."""
    songs = await _ytm_search_songs(q, limit=limit)
    items = [
        _fm_song(s["video_id"], s.get("title") or "", s.get("artist") or "", s.get("thumb"))
        for s in songs
    ]
    return {
        "ok": True,
...
@app.get("/fastmusic/suggestions", tags=["FastMusic"])
@app.get("/pax/suggestions", tags=["FastMusic"])
async def fm_suggestions(q: str = Query(..., min_length=1)):
    """Search Suggestions."""
    # use search and return titles as suggestions + query variants
    try:
        songs = await _ytm_search_songs(q, limit=10)
        suggestions = []
        seen = set()
        for s in songs:
            ...
            if a and f"{t} {a}".strip().lower() not in seen:
                suggestions.append(f"{t} - {a}")
        return {"ok": True, "query": q, "suggestions": suggestions[:15], "provider": "fastmusic"}
    except Exception as e:
        return {"ok": False, "query": q, "suggestions": [q], "error": str(e)[:80]}
...
async def fm_home():
    """Home feed sections (charts + seeds)."""
    try:
        sections = await _ytm_home_sections()
    except Exception:
        sections = []
    ...
    for sec in sections:
        ...
        if items:
            out.append({"title": sec.get("title") or "Music", "items": items, "contents": items})
    return {"ok": bool(out), "sections": out, "result": out, "provider": "fastmusic"}
...
async def fm_next(
    id: Optional[str] = Query(None, description="video id"),
    videoId: Optional[str] = Query(None, description="video id alias"),
    video_id: Optional[str] = Query(None),
    limit: int = Query(25, ge=1, le=50),
):
    """Next Music / radio queue (RDAMVM)."""
...
@app.get("/fastmusic/related", tags=["FastMusic"])
@app.get("/pax/related", tags=["FastMusic"])
async def fm_related(id: str = Query(..., description="video id"), limit: int = Query(20, ge=1, le=50)):
    """Get related content — same radio mix."""
...
@app.get("/fastmusic/info", tags=["FastMusic"])
@app.get("/pax/info", tags=["FastMusic"])
async def fm_info(id: str = Query(..., description="video id")):
    """Get Music info — metadata + stream-ready fields."""
    vid = id.replace("yt:", "").strip()
    title, artist, thumb, duration = vid, "", f"https://i.ytimg.com/vi/{vid}/hqdefault.jpg", None
    # try next for meta
    try:
    ...
    try:
        ...
        pass
    song = _fm_song(vid, title, artist, thumb, duration=duration)
    song["lengthSeconds"] = duration
    song["duration"] = duration
    return {"ok": True, "result": song, **song, "provider": "fastmusic"}
...
async def fm_play(
    id: str = Query(..., description="video id"),
    format: str = Query("audio", description="audio|video|mp3|360|720"),
):
    """Stream CDN — replaces dead youtube-data.deno.dev/download?id=

    Returns direct googlevideo URL for ExoPlayer (title, author, thumbnail, url).
    """
    vid = id.replace("yt:", "").strip()
    if not re.match(r"^[\w-]{6,20}$", vid):
        raise HTTPException(400, "invalid video id")
    data = await _innertube_player(vid)
    fmt = (format or "audio").lower()
    stream_url = None
    if data.get("ok"):
        if fmt in ("audio", "mp3", "m4a", "bestaudio"):
    ...
    if not stream_url:
        # loader fallback
        try:
            ld = await _loader_to_youtube(vid, fmt="mp3" if "audio" in fmt or fmt == "mp3" else "360")
            if ld.get("ok") and ld.get("url"):
                stream_url = ld["url"]
                data = {
                    "ok": True,
            ...
            pass
    if not stream_url:
        raise HTTPException(502, data.get("error") if isinstance(data, dict) else "stream unavailable")

    title = (data.get("title") if isinstance(data, dict) else None) or vid
    thumb = (data.get("thumb") if isinstance(data, dict) else None) or f"https://i.ytimg.com/vi/{vid}/hqdefault.jpg"
    # Sketchware player fields
    return {
        "ok": True,
        "videoId": vid,
        "video_id": vid,
        ...
        "audio_url": data.get("audio_url") if isinstance(data, dict) else stream_url,
        ...
        "format": format,
        "provider": (data.get("provider") if isinstance(data, dict) else None) or "innertube",
        "note": "Direct CDN for ExoPlayer — tokens expire ~6h",
    }


@app.get("/fm/lyrics", tags=["FastMusic"])
@app.get("/fastmusic/lyrics", tags=["FastMusic"])
...
async def fm_lyrics(
    ...
    async with httpx.AsyncClient(timeout=15.0) as client:
        ...
        arr = r.json() if r.status_code == 200 else []
    if not arr:
        return {"ok": False, "videoId": id, "title": t, "artist": a, "lyrics": None, "synced": None}
    best = arr[0]
...
async def fm_match_song(
    title: str = Query(..., min_length=1),
    artist: str = Query(""),
):
    """Find match song — search best match."""
    q = f"{title} {artist}".strip()
    songs = await _ytm_search_songs(q, limit=5)
    if not songs:
        return {"ok": False, "query": q, "result": None}
    s = songs[0]
    return {
        "ok": True,
        "query": q,
        "result": _fm_song(s["video_id"], s.get("title") or "", s.get("artist") or "", s.get("thumb")),
        "provider": "fastmusic",
    }
...
async def fm_trending(limit: int = Query(20, ge=1, le=40)):
    songs = await _ytm_search_songs("Top songs this week", limit=limit)
    items = [_fm_song(s["video_id"], s.get("title") or "", s.get("artist") or "", s.get("thumb")) for s in songs]
    return {"ok": bool(items), "items": items, "result": items, "count": len(items), "provider": "fastmusic"}


@app.get("/fm/playlist", tags=["FastMusic"])
@app.get("/pax/playlist", tags=["FastMusic"])
async def fm_playlist(id: str = Query(..., description="playlist id")):
    """Playlist tracks via browse (best-effort)."""
    try:
        data = await _ytm_post("browse", {"browseId": id if id.startswith("VL") or id.startswith("PL") else f"VL{id}"})
        items_raw = []
        _ytm_walk(data, "musicResponsiveListItemRenderer", items_raw)
        songs = []
        seen = set()
        for it in items_raw:
            e = _ytm_parse_item(it)
            if e and e["video_id"] not in seen:
                seen.add(e["video_id"])
                songs.append(_fm_song(e["video_id"], e.get("title") or "", e.get("artist") or "", e.get("thumb")))
...
async def fm_album(id: str = Query(...)):
    ...
    try:
        ...
        return {"ok": False, "albumId": id, "error": str(e)[:120], "items": []}


@app.get("/fm/artist", tags=["FastMusic"])
@app.get("/pax/artist", tags=["FastMusic"])
async def fm_artist(id: str = Query(..., description="channel / browse id")):
    try:
        data = await _ytm_post("browse", {"browseId": id})
        items_raw = []
        _ytm_walk(data, "musicResponsiveListItemRenderer", items_raw)
        ...
        for it in items_raw:
            ...
            if e and e["video_id"] not in seen:
                seen.add(e["video_id"])
                songs.append(_fm_song(e["video_id"], e.get("title") or "", e.get("artist") or "", e.get("thumb")))
        return {"ok": bool(songs), "artistId": id, "items": songs[:30], "result": songs[:30], "provider": "fastmusic"}
    except Exception as e:

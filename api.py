            ...
            for i in range(len(MB_HOSTS)):
                ...
                try:
                    ...
                    if resp.status_code in (401, 403) and attempt == 0:
                        ...
                        await _mb_login(client)
                        break
                    if resp.status_code in RETRY_CODES or not (200 <= resp.status_code < 300):
                        continue
                    data = resp.json()
                    _mb_idx = idx
                    if isinstance(data, dict):
                        payload = data.get("data", data)
                        # Some BFF versions wrap the payload twice.
...
async def _piped_yt_streams(video_id: str) -> dict:
    ...
    # Keep several public instances because these services frequently rotate
    # or temporarily rate-limit individual hosts.
    bases = [
        ...
        for base in bases:
            try:
                ...
                return {
                    ...
                    "cdn_url": formats[0]["url"],
                    "directUrl": formats[0]["url"],
                    "cdnUrl": formats[0]["url"],
                    "hls": j.get("hls"),
                    "dash": j.get("dash"),
...
async def _resolve_media_info(url: str) -> dict:
    """Resolve a URL to usable direct media URLs.

    yt-dlp is the primary extractor.  YouTube frequently blocks datacenter
    IPs, and some deployments do not ship yt-dlp at all, so a YouTube URL is
    retried through public Piped instances before falling back to an embed.
    This helper is intentionally used by every downloader endpoint so their
    behavior is consistent.
    """
    value = (url or "").strip()
    data = await asyncio.to_thread(_ytdlp_info, value)
    if data.get("ok") and data.get("formats"):
        return data
    yt_id = _extract_youtube_id(value)
    if yt_id:
        piped = await _piped_yt_streams(yt_id)
        if piped.get("ok") and piped.get("formats"):
            return piped
        # Preserve the useful yt-dlp error when both providers fail.
        if piped.get("detail"):
            data["fallback_error"] = piped["detail"]
    return data
...
async def dl_info(url: str = Query(..., min_length=8)):
    ...
    data = await _resolve_media_info(url)
    if not data.get("ok"):
...
async def dl_audio(url: str = Query(..., min_length=8)):
    ...
    return {
        ...
        "title": data.get("title"),
        "audio": best,
        "audio_url": best.get("url") if best else None,
        "directUrl": best.get("url") if best else None,
        "cdnUrl": best.get("url") if best else None,
...
async def dl_smart(url: str = Query(..., min_length=8)):
    ...
    if muxed:
        return {
            ...
            "directUrl": muxed[0].get("url"),
            "cdnUrl": muxed[0].get("url"),
            "format": muxed[0],
    ...
    if videos:
        return {"ok": True, "mode": "video_only", "title": title, "download_url": videos[0].get("url"), "directUrl": videos[0].get("url"), "cdnUrl": videos[0].get("url"), "format": videos[0]}
    if audios:
        return {"ok": True, "mode": "audio_only", "title": title, "download_url": audios[0].get("url"), "directUrl": audios[0].get("url"), "cdnUrl": audios[0].get("url"), "format": audios[0]}
...
async def dl_any(url: str = Query(..., min_length=8)):
    ...
    data = await _resolve_media_info(url)
    if not data.get("ok"):
        return JSONResponse({"ok": False, "provider": "yt-dlp+piped", "error": data.get("error"), "input": url}, status_code=422)
    ...
    result = {
        "ok": True,
        "provider": "piped" if str(data.get("extractor") or "").startswith("piped/") else "yt-dlp",
    ...
    if muxed:
        ...
        result["directUrl"] = muxed[0].get("url")
        result["cdnUrl"] = muxed[0].get("url")
        result["best"] = muxed[0]

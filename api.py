...
# ---- YouTube player cache (speed up /download) ----
_innertube_cache: Dict[str, Any] = {}  # video_id -> {ts, data}
_INNERTUBE_CACHE_TTL = 300  # 5 min
...
async def health():
    return {"ok": True, "version": "5.39.0", "creator": "shawon", "providers": ["hindianime", "ytmusic", "deezer", "jiosaavn", "tmdb"]}
...
async def _loader_to_youtube(url_or_id: str, fmt: str = "mp3") -> dict:
    """loader.to → savenow CDN. Poll progress_url until ready (works when InnerTube bot-blocked)."""
    vid = _extract_youtube_id(url_or_id) or (
        url_or_id if re.match(r"^[\w-]{6,20}$", (url_or_id or "").strip()) else None
    )
    if not vid:
    ...
    try:
        async with httpx.AsyncClient(timeout=20.0, follow_redirects=True) as client:
            r = await client.get(
                ...
                params={"format": fmt if fmt in ("mp3", "m4a", "360", "480", "720", "1080") else "mp3", "url": watch},
                headers=headers,
            )
            if r.status_code != 200:
                return {"ok": False, "error": f"loader init HTTP {r.status_code}"}
            try:
                j = r.json()
            except Exception:
                return {"ok": False, "error": "loader non-json"}
            if not j.get("success") and not j.get("id"):
                return {"ok": False, "error": j.get("text") or j.get("message") or "loader failed"}
            pid = j.get("id")
            prog_url = (j.get("progress_url") or "").replace("\\/", "/")
            if not prog_url and pid:
                prog_url = f"https://lto2.affadaffa.com/api/progress?id={pid}"
            urls_try = []
            if prog_url:
                urls_try.append(prog_url)
            if pid:
                urls_try.extend([
                    f"https://lto2.affadaffa.com/api/progress?id={pid}",
                    f"https://loader.to/ajax/progress.php?id={pid}",
                    f"https://p.oceansaver.in/ajax/progress.php?id={pid}",
                ])
            # dedupe
            seen = set()
            urls_try = [u for u in urls_try if u and not (u in seen or seen.add(u))]
            download_url = None
            title = None
            for _ in range(18):  # ~18 * 0.9s ≈ 16s max
                for u in urls_try:
                    try:
                        p = await client.get(u, headers=headers)
                        if p.status_code != 200:
                            continue
                        pj = p.json()
                        du = pj.get("download_url") or pj.get("url") or pj.get("download")
                        # progress 1000 = done on lto2
                        if du and (pj.get("success") in (1, True, "1") or int(pj.get("progress") or 0) >= 1000 or du.startswith("http")):
                            download_url = du
            ...
            if not download_url:
                return {"ok": False, "error": "loader timeout — try again", "provider": "loader.to"}
            return {
                "ok": True,
                "url": download_url,
...
async def _innertube_player(video_id: str, prefer: str = "auto") -> dict:
    """Native YouTube InnerTube player — fast path + cache.

    Tries ANDROID / IOS in parallel first (~0.2s). Caches OK results 5 min.
    """
    ...
    try:
        hit = _innertube_cache.get(video_id)
        if hit and (time.time() - hit.get("ts", 0)) < _INNERTUBE_CACHE_TTL:
            data = hit.get("data") or {}
            if data.get("ok") or (time.time() - hit.get("ts", 0)) < 30:
                return data
    except Exception:
        pass
    ...
    def _parse_player(j: dict, cname: str) -> Optional[dict]:
        ...
        try:
            ...
            pass
        audio_streams, video_streams, formats = [], [], []
        audio_url = video_url = None
        for f in raw:
            u = f.get("url")
            if not u:
                continue  # skip cipher-only
            ...
            if "audio" in mime and "video" not in mime:
                entry["quality"] = f.get("audioQuality")
                audio_streams.append(entry)
                formats.append({**entry, "kind": "audio"})
                if not audio_url:
                    audio_url = u
                elif "AUDIO_QUALITY_MEDIUM" in str(f.get("audioQuality") or "") or (f.get("bitrate") or 0) > 128000:
                    audio_url = u
            else:
                h = f.get("height") or 0
                entry["height"] = h
                entry["width"] = f.get("width")
                entry["quality"] = f.get("qualityLabel") or f.get("quality")
                entry["progressive"] = "video" in mime and "audio" in mime
                video_streams.append(entry)
                formats.append({**entry, "kind": "video"})
                if entry["progressive"] and (not video_url or h >= 360):
                    video_url = u
                elif not video_url and h:
                    video_url = u
        ...
        return {
            ...
            "formats": formats[:40],
            "provider": f"innertube-{cname}",
        }

    async def _try_client(client: httpx.AsyncClient, c: dict) -> Optional[dict]:
        body = {
            "context": {
                "client": {
        ...
        try:
            ...
            if r.status_code != 200:
                return None
            return _parse_player(r.json(), c["name"])
        except Exception:
            return None

    last_err = "no stream"
    async with httpx.AsyncClient(timeout=8.0, follow_redirects=True) as client:
        # parallel race first two clients
        first = clients[:2]
        results = await asyncio.gather(*[_try_client(client, c) for c in first], return_exceptions=True)
        for res in results:
            if isinstance(res, dict) and res.get("ok"):
                try:
                    _innertube_cache[video_id] = {"ts": time.time(), "data": res}
                except Exception:
                    pass
                return res
        # sequential fallback remaining
        for c in clients[2:]:
            res = await _try_client(client, c)
            if res and res.get("ok"):
                try:
                    ...
                    pass
                return res
            last_err = f"{c['name']} failed"

    out = {"ok": False, "error": last_err or "LOGIN_REQUIRED / no stream", "video_id": video_id}
    try:
        _innertube_cache[video_id] = {"ts": time.time(), "data": out}
    except Exception:
        pass
    return out
...
@app.get("/youtube/play/{video_id}", tags=["YouTube"])
...
# =============================================================================
# ClulesVibe (PaxSenix Sketchware) — exact RapidAPI path drop-ins (unlimited native)
# From decrypted logic:
#   youtube-music-api3.p.rapidapi.com/search?q=&type=song|artists
#   /v2/home  /next?id=  /music/lyrics/plain?id=
#   /getAlbum?id=  /getArtists?id=
#   musixmatch-lyrics-songs .../songs/lyrics
#   youtube-data.deno.dev/download?id=  /thumbnailHD?id=
# App reads: videoId, title, author, thumbnail, duration, browseId, channelId
# =============================================================================
...
def _cv_item(video_id: str, title: str = "", author: str = "", thumb: str = None, duration=None, **extra) -> dict:
    ...
    t2 = f"https://i.ytimg.com/vi/{vid}/mqdefault.jpg" if vid else None
    out = {
        "videoId": vid,
        "title": title or vid,
...
@app.get("/search", tags=["ClulesVibe"])
async def cv_search(
    q: str = Query(..., min_length=1),
    type: str = Query("song", description="song|artists|videos|albums|playlists"),
    limit: int = Query(20, ge=1, le=50),
):
    """RapidAPI youtube-music-api3 /search — ClulesVibe SearchFragment."""
    t = (type or "song").lower()
    if t in ("artist", "artists"):
        # search songs then map artists best-effort, or search as songs with type filter
        songs = await _ytm_search_songs(q, limit=limit)
        # unique authors as artist-ish cards
        seen = set()
        items = []
        for s in songs:
...
@app.get("/v2/home", tags=["ClulesVibe"])
async def cv_v2_home():
    """RapidAPI /v2/home — Quick Picks / home sections."""
    sections = []
    try:
        home = await _ytm_home_sections()
        for sec in home:
            items = [
    ...
    if not sections:
        for title, q in [("You might also like", "Top songs this week"), ("Trending", "Trending music"), ("Quick picks", "pop hits")]:
            try:
                ...
                continue
    # flat result also for simple parsers
    flat = []
    for s in sections:
...
@app.get("/next", tags=["ClulesVibe"])
async def cv_next(id: str = Query(..., description="videoId")):
    """RapidAPI /next?id= — Next Song queue."""
    vid = id.replace("yt:", "").strip()
    songs = await _ytm_next_songs(vid, limit=25)
    items = [
        _cv_item(s["video_id"], s.get("title") or "", s.get("artist") or "", s.get("thumb"))
        for s in songs
    ]
...
@app.get("/music/lyrics/plain", tags=["ClulesVibe"])
async def cv_lyrics_plain(id: str = Query(..., description="videoId")):
    """RapidAPI /music/lyrics/plain?id="""
    vid = id.replace("yt:", "").strip()
    title, artist = vid, ""
    try:
        pl = await _innertube_player(vid)
        if pl.get("ok"):
...
@app.get("/getAlbum", tags=["ClulesVibe"])
async def cv_get_album(id: str = Query(..., description="album browseId")):
    """RapidAPI /getAlbum?id="""
    try:
        data = await _ytm_post("browse", {"browseId": id})
        raw = []
        _ytm_walk(data, "musicResponsiveListItemRenderer", raw)
...
@app.get("/getArtists", tags=["ClulesVibe"])
@app.get("/getArtist", tags=["ClulesVibe"])
async def cv_get_artists(id: str = Query(..., description="channel/browse id")):
    """RapidAPI /getArtists?id="""
    try:
        data = await _ytm_post("browse", {"browseId": id})
        raw = []
        _ytm_walk(data, "musicResponsiveListItemRenderer", raw)
...
@app.get("/songs/lyrics", tags=["ClulesVibe"])
async def cv_musixmatch_lyrics(
    track_id: Optional[str] = Query(None),
    id: Optional[str] = Query(None),
    t: Optional[str] = Query(None, alias="type"),
    q: Optional[str] = Query(None),
    title: Optional[str] = Query(None),
    artist: Optional[str] = Query(""),
):
    """Musixmatch RapidAPI shape used by ClulesVibe (type=text)."""
...
# =============================================================================
# FastMusic — PaxSenix Sketchware app compatible (unlimited native)
# App ExoPlayer uses:
#   Uri.parse("…/download?id=" + videoId + "&type=audio&quality=best")
# → must be PLAYABLE media (302 to googlevideo) NOT JSON
# List APIs return: videoId, title, author, thumbnail (+ optional thumbnail2)
# =============================================================================
...
def _fm_item(video_id: str, title: str = "", author: str = "", thumb: str = None, **extra) -> dict:
    ...
    return {
        ...
        # stream endpoints the app can hit
...
async def _fm_resolve_stream(vid: str, type_: str = "audio", quality: str = "best") -> dict:
    """Return {ok, url, title, thumb, mime, provider} for ExoPlayer. Fast path: cache + music client."""
    # prefer music client for audio (usually 1 request)
    prefer = "music" if (type_ or "audio").lower() in ("audio", "mp3", "m4a", "bestaudio") else "auto"
    data = await _innertube_player(vid, prefer=prefer)
    try:
        if data.get("ok"):
            _innertube_cache[vid] = {"ts": time.time(), "data": data}
    except Exception:
        pass
    stream_url = None
    mime = "audio/mp4"
    if data.get("ok"):
        ...
        if t in ("audio", "mp3", "m4a", "bestaudio") or q in ("audio", "mp3"):
            ...
            # pick best audio from streams if needed
            if not stream_url:
    ...
    if not stream_url:
        try:
            ld = await asyncio.wait_for(
                _loader_to_youtube(vid, fmt="mp3" if "audio" in (type_ or "audio").lower() else "360"),
                timeout=20.0,
            )
            if ld.get("ok") and ld.get("url"):
                return {
...
# ---- thumbnailHD (PaxSenix / youtube-data.deno.dev drop-in) ----
@app.get("/thumbnailHD", tags=["FastMusic"])
@app.get("/fm/thumbnailHD", tags=["FastMusic"])
@app.get("/pax/thumbnailHD", tags=["FastMusic"])
@app.get("/api/thumbnailHD", tags=["FastMusic"])
async def fm_thumbnail_hd(
    id: str = Query(..., description="YouTube video id"),
    redirect: bool = Query(True, description="302 to i.ytimg.com"),
    proxy: bool = Query(False, description="stream image bytes (Glide-safe)"),
):
    """Drop-in for youtube-data.deno.dev/thumbnailHD?id=

    Prefers maxres when real (>5KB), else hqdefault — avoids gray placeholder.
    ?proxy=1 streams JPEG through this API (best for some Android Glide builds).
    """
    ...
    chosen = candidates[2]  # safe default hq
    try:
        async with httpx.AsyncClient(timeout=4.0, follow_redirects=True) as client:
            for u in candidates:
                try:
                    ...
                    # real maxres/sd usually > 15KB; placeholder ~1-2KB
...
@app.get("/thumbnail", tags=["FastMusic"])
@app.get("/fm/thumbnail", tags=["FastMusic"])
async def fm_thumbnail(
    id: str = Query(...),
    quality: str = Query("hq", description="max|sd|hq|mq|default"),
):
    """Simple thumbnail redirect (non-HD variants)."""
    ...
    name = qmap.get((quality or "hq").lower(), "hqdefault")
    return RedirectResponse(url=f"https://i.ytimg.com/vi/{vid}/{name}.jpg", status_code=302)


@app.get("/download", tags=["FastMusic"])
@app.get("/fm/download", tags=["FastMusic"])
@app.get("/api/download", tags=["FastMusic"])
@app.get("/pax/download", tags=["FastMusic"])
@app.get("/fastmusic/download", tags=["FastMusic"])
async def fm_download_playable(
    id: str = Query(..., description="YouTube video id"),
    type: str = Query("audio", description="audio|video"),
    quality: str = Query("best", description="best|360|480|720|1080|audio"),
    redirect: bool = Query(True, description="302 to CDN (ExoPlayer friendly)"),
    as_json: bool = Query(False, alias="json", description="force JSON meta instead of redirect"),
):
    """
    Drop-in replacement for:
      https://youtube-data.deno.dev/download?id=VIDEO_ID&type=audio&quality=best

    Default: HTTP 302 → googlevideo CDN (ExoPlayer plays directly).
    ?json=1 returns metadata + url without redirect.
    """
    vid = (id or "").replace("yt:", "").strip()
    if not re.match(r"^[\w-]{6,20}$", vid):
        raise HTTPException(400, "invalid video id")
    resolved = await _fm_resolve_stream(vid, type_=type, quality=quality)
    if not resolved.get("ok"):
        raise HTTPException(502, resolved.get("error") or "stream unavailable")
    stream_url = resolved["url"]
    if as_json or not redirect:
        return {
            "ok": True,
            "videoId": vid,
    ...
    # 302 so ExoPlayer follows to googlevideo
    from fastapi.responses import RedirectResponse
    return RedirectResponse(url=stream_url, status_code=302)


@app.get("/fm/play", tags=["FastMusic"])
@app.get("/pax/play", tags=["FastMusic"])
@app.get("/fastmusic/play", tags=["FastMusic"])
async def fm_play_json(
    id: str = Query(...),
    type: str = Query("audio"),
    quality: str = Query("best"),
):
    """JSON stream info (for debugging / non-Exo clients)."""
    return await fm_download_playable(id=id, type=type, quality=quality, redirect=False, as_json=True)


# ---- SEARCH ----
@app.get("/fm/search", tags=["FastMusic"])
@app.get("/pax/search", tags=["FastMusic"])
@app.get("/fastmusic/search", tags=["FastMusic"])
@app.get("/search/music", tags=["FastMusic"])
async def fm_search(
    q: str = Query(..., min_length=1),
    type: str = Query("song"),
    limit: int = Query(20, ge=1, le=50),
    ...
    songs = await _ytm_search_songs(q, limit=limit)
    items = [
        _fm_item(s["video_id"], s.get("title") or "", s.get("artist") or "", s.get("thumb"))
        for s in songs
    ]
    return {
        "ok": True,
...
@app.get("/fm/suggestions", tags=["FastMusic"])
@app.get("/pax/suggestions", tags=["FastMusic"])
...
# ---- HOME (sections like "You might also like") ----
@app.get("/fm/home", tags=["FastMusic"])
@app.get("/pax/home", tags=["FastMusic"])
@app.get("/fastmusic/home", tags=["FastMusic"])
async def fm_home():
    sections = []
    seeds = [
        ("You might also like", "Top songs this week"),
    ...
    for title, q in seeds:
        try:
            ...
            if items:
                sections.append({"title": title, "items": items, "contents": items})
        except Exception:
            continue
    # also try real home browse
    try:
        home = await _ytm_home_sections()
        for sec in home:
            items = [
            ...
            if items and not any(x["title"] == sec.get("title") for x in sections):
                sections.append({"title": sec.get("title") or "Home", "items": items, "contents": items})
    except Exception:
        pass
    return {
        "ok": bool(sections),
...
# ---- NEXT / RELATED ----
@app.get("/fm/next", tags=["FastMusic"])
@app.get("/fm/next-v2", tags=["FastMusic"])
@app.get("/pax/next", tags=["FastMusic"])
@app.get("/fastmusic/next", tags=["FastMusic"])
async def fm_next(
    id: Optional[str] = Query(None),
    ...
    songs = await _ytm_next_songs(vid, limit=limit)
    items = [
        _fm_item(s["video_id"], s.get("title") or "", s.get("artist") or "", s.get("thumb"))
        for s in songs
    ]
...
@app.get("/fm/related", tags=["FastMusic"])
@app.get("/pax/related", tags=["FastMusic"])
...
# ---- INFO ----
@app.get("/fm/info", tags=["FastMusic"])
@app.get("/fm/info-v2", tags=["FastMusic"])
@app.get("/fm/get", tags=["FastMusic"])
@app.get("/pax/info", tags=["FastMusic"])
async def fm_info(id: str = Query(...)):
    ...
    try:
        ...
        pass
    try:
        data = await _ytm_post("next", {"videoId": vid, "isAudioOnly": True})
        texts = re.findall(r'"text"\s*:\s*"([^"\\]{1,120})"', json.dumps(data))
        if texts:
            title = texts[0] or title
            if len(texts) > 1:
                author = texts[1]
    except Exception:
        pass
...
# ---- LYRICS ----
@app.get("/fm/lyrics", tags=["FastMusic"])
@app.get("/pax/lyrics", tags=["FastMusic"])
async def fm_lyrics(
    id: Optional[str] = Query(None),
    title: Optional[str] = Query(None),
    artist: Optional[str] = Query(""),
    ...
    if id and not t:
        try:
            info = await fm_info(id=id)
            t = info.get("title") or id
            a = info.get("author") or a
        except Exception:
            t = id or ""
    if not t:
        raise HTTPException(400, "id or title required")
    async with httpx.AsyncClient(timeout=15.0) as client:
        r = await client.get("https://lrclib.net/api/search", params={"q": f"{t} {a}".strip()})
        arr = r.json() if r.status_code == 200 else []
    if not arr:
        return {"ok": False, "videoId": id, "title": t, "lyrics": None, "synced": None}
    best = arr[0]
    ...
    return {
        "ok": True,
        "videoId": id,
        "title": best.get("trackName") or t,
        "artist": best.get("artistName") or a,
        "author": best.get("artistName") or a,
...
@app.get("/fm/match", tags=["FastMusic"])
@app.get("/pax/match", tags=["FastMusic"])
async def fm_match(title: str = Query(...), artist: str = Query("")):
    songs = await _ytm_search_songs(f"{title} {artist}".strip(), limit=5)
    if not songs:
        return {"ok": False, "result": None}
    s = songs[0]
    return {
        "ok": True,
        "result": _fm_item(s["video_id"], s.get("title") or "", s.get("artist") or "", s.get("thumb")),
        "provider": "fastmusic",
    }


@app.get("/fm/trending", tags=["FastMusic"])
@app.get("/pax/trending", tags=["FastMusic"])
...
@app.get("/fm/playlist", tags=["FastMusic"])
@app.get("/pax/playlist", tags=["FastMusic"])
async def fm_playlist(id: str = Query(...)):
    try:
        bid = id if id.startswith(("VL", "PL", "RD")) else f"VL{id}"
        data = await _ytm_post("browse", {"browseId": bid})
        raw = []
        _ytm_walk(data, "musicResponsiveListItemRenderer", raw)
        songs, seen = [], set()
        for it in raw:
            ...
            if e and e["video_id"] not in seen:
                seen.add(e["video_id"])
                songs.append(_fm_item(e["video_id"], e.get("title") or "", e.get("artist") or "", e.get("thumb")))
        return {"ok": bool(songs), "playlistId": id, "items": songs, "result": songs, "count": len(songs), "provider": "fastmusic"}
    except Exception as e:
        return {"ok": False, "playlistId": id, "items": [], "error": str(e)[:120]}


@app.get("/fm/album", tags=["FastMusic"])
@app.get("/pax/album", tags=["FastMusic"])
    ...
    try:
        ...
        for it in raw:
            ...
            if e and e["video_id"] not in seen:
                seen.add(e["video_id"])
                songs.append(_fm_item(e["video_id"], e.get("title") or "", e.get("artist") or "", e.get("thumb")))
        return {"ok": bool(songs), "albumId": id, "items": songs, "result": songs, "count": len(songs), "provider": "fastmusic"}
    except Exception as e:
        return {"ok": False, "albumId": id, "items": [], "error": str(e)[:120]}


@app.get("/fm/artist", tags=["FastMusic"])
@app.get("/pax/artist", tags=["FastMusic"])
    ...
    try:
        ...
        for it in raw:
            ...
            if e and e["video_id"] not in seen:
                seen.add(e["video_id"])
                songs.append(_fm_item(e["video_id"], e.get("title") or "", e.get("artist") or "", e.get("thumb")))
        return {"ok": bool(songs), "artistId": id, "items": songs[:40], "result": songs[:40], "provider": "fastmusic"}
    except Exception as e:
        return {"ok": False, "artistId": id, "error": str(e)[:120]}

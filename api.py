...
async def health():
    return {"ok": True, "version": "5.36.0", "creator": "shawon", "providers": ["hindianime", "ytmusic", "deezer", "jiosaavn", "tmdb"]}
...
async def _fm_resolve_stream(vid: str, type_: str = "audio", quality: str = "best") -> dict:
    """Return {ok, url, title, thumb, mime, provider} for ExoPlayer."""
    data = await _innertube_player(vid)
    stream_url = None
    mime = "audio/mp4"
    if data.get("ok"):
    ...
    if not stream_url:
        try:
            ld = await _loader_to_youtube(vid, fmt="mp3" if "audio" in (type_ or "audio").lower() else "360")
            if ld.get("ok") and ld.get("url"):
                return {
                    "ok": True,
                    "url": ld["url"],
